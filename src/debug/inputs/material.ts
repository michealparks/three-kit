...
import { Panes, addFolder, deleteFolder } from '../pane'
import { defaultMinMax } from '../constants'

export const addMaterialInputs = (pane: Panes, mesh: THREE.Mesh) => {
  const materialFolder = addFolder(pane, `#${mesh.id} ${mesh.material.type}`)
  const { material } = mesh
